...
const InteractiveRealEstateMap = ({ onInteraction, onCountySelected }) => {
  ...
  // Function to update county details in DOM
  ...
  // Add a direct DOM update effect
  useEffect(() => {
    if (selectedCounty && typeof window !== 'undefined') {
      const countyDetailsContainer = document.getElementById('county-details-container');
      ...
      if (countyDetailsContainer) {
        ...
        // Create county details HTML directly
        const countyDetailsHTML = `
          <div class="p-6 county-details" data-county="${county.name}">
            <div class="mb-8 border-b border-gray-200 pb-6">
              <h2 class="text-2xl font-bold text-gray-800 mb-2 flex items-center">
                <span class="w-4 h-4 rounded-full mr-2" style="background-color: ${county.color}"></span>
                ${county.name} County
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-gradient-to-br from-blue-50 to-blue-100 p-5 rounded-xl shadow-sm">
                  <h3 class="text-sm font-medium text-blue-800 uppercase tracking-wide mb-2">Population</h3>
                  <p class="text-2xl font-bold text-gray-800">${county.details.population.toLocaleString()}</p>
                </div>
                <div class="bg-gradient-to-br from-indigo-50 to-indigo-100 p-5 rounded-xl shadow-sm">
                  <h3 class="text-sm font-medium text-indigo-800 uppercase tracking-wide mb-2">Median Home Price</h3>
                  <p class="text-2xl font-bold text-gray-800">${county.details.medianHomePrice}</p>
                </div>
                <div class="bg-gradient-to-br from-purple-50 to-purple-100 p-5 rounded-xl shadow-sm">
                  <h3 class="text-sm font-medium text-purple-800 uppercase tracking-wide mb-2">Average Income</h3>
                  <p class="text-2xl font-bold text-gray-800">${county.details.averageIncomePerHousehold}</p>
                </div>
              </div>
                    ...
                    <div class="p-4">
                      ...
                      <div class="mt-3 flex items-center justify-between text-sm">
                        ...
                        <div class="flex items-center text-gray-600">
                          ...
                          <span>${(property.LivingArea || 'N/A')} sqft</span>
                        </div>
                      </div>
                      <div class="mt-4 pt-3 border-t border-gray-100">
                        <button class="w-full text-blue-600 text-sm font-medium hover:text-blue-800 transition-colors">
        ...
        // Get the content div (or create it if it doesn't exist)
        let contentDiv = countyDetailsContainer.querySelector('.county-details-content');
        if (!contentDiv) {
          // Clear the loading state first
          countyDetailsContainer.innerHTML = '';
          contentDiv = document.createElement('div');
          contentDiv.className = 'county-details-content';
          countyDetailsContainer.appendChild(contentDiv);
        }
        ...
        const activeBtn = contentDiv.querySelector('#active-listings-btn');
        const closedBtn = contentDiv.querySelector('#closed-listings-btn');
        ...
        // Add event listeners to property cards
        contentDiv.querySelectorAll('.property-card').forEach(card => {
          card.addEventListener('click', () => {
            const propertyId = card.getAttribute('data-property-id');
            if (propertyId) {
              handlePropertyClick(propertyId);
  ...
  }, [selectedCounty, propertiesByCounty, selectedStatus, loading, error]);
  ...
  const handleCountyClick = (countyFips) => {
    ...
    if (countyDetailsContainer) {
      ...
      const loadingHTML = `
        <div class="county-details-loading p-6">
          <div class="mb-8 border-b border-gray-100 pb-6">
            ...
            <div class="animate-pulse bg-gray-200 h-4 rounded w-3/4 mb-2"></div>
            <div class="animate-pulse bg-gray-200 h-4 rounded w-full mb-2"></div>
          ...
          <div class="mt-8">
            ...
            <div class="flex justify-center mt-8 mb-4">
              <div class="animate-bounce flex items-center space-x-2 text-blue-600">
                <div class="h-2 w-2 rounded-full bg-blue-600"></div>
                <div class="h-2 w-2 rounded-full bg-blue-600 animate-delay-100"></div>
                <div class="h-2 w-2 rounded-full bg-blue-600 animate-delay-200"></div>
                <span class="text-sm font-medium ml-2">Loading Properties</span>
              </div>
            </div>
          </div>
        </div>
      ...
      // Set loading content with skeleton UI
      contentDiv.innerHTML = loadingHTML;
    }
  };

  // Add a direct DOM update effect with smooth transitions
  useEffect(() => {
    if (selectedCounty && typeof window !== 'undefined') {
      const countyDetailsContainer = document.getElementById('county-details-container');
      
      if (countyDetailsContainer && !loading) {
        // Get county data
        const county = COUNTY_STYLES[selectedCounty];
        const properties = propertiesByCounty[selectedCounty]?.[selectedStatus] || [];
        
        // Create county details HTML with transition effect
        const countyDetailsHTML = `
          <div class="p-6 county-details fade-in" data-county="${county.name}">
            <div class="mb-8 border-b border-gray-200 pb-6">
              <h2 class="text-2xl font-bold text-gray-800 mb-2 flex items-center">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-gradient-to-br from-blue-50 to-blue-100 p-5 rounded-xl shadow-sm slide-in" style="animation-delay: 50ms">
                  <h3 class="text-sm font-medium text-blue-800 uppercase tracking-wide mb-2">Population</h3>
                  <p class="text-2xl font-bold text-gray-800">${county.details.population.toLocaleString()}</p>
                </div>
                <div class="bg-gradient-to-br from-indigo-50 to-indigo-100 p-5 rounded-xl shadow-sm slide-in" style="animation-delay: 100ms">
                  <h3 class="text-sm font-medium text-indigo-800 uppercase tracking-wide mb-2">Median Home Price</h3>
                  <p class="text-2xl font-bold text-gray-800">${county.details.medianHomePrice}</p>
                </div>
                <div class="bg-gradient-to-br from-purple-50 to-purple-100 p-5 rounded-xl shadow-sm slide-in" style="animation-delay: 150ms">
                  <h3 class="text-sm font-medium text-purple-800 uppercase tracking-wide mb-2">Average Income</h3>
                  <p class="text-2xl font-bold text-gray-800">${county.details.averageIncomePerHousehold}</p>
                </div>
              </div>
                    ...
                    <div class="p-4">
                      ...
                      <div class="mt-3 flex items-center justify-between text-sm">
                        ...
                        <div class="flex items-center text-gray-600">
                          ...
                          <span>${(property.LivingArea || 'N/A').toLocaleString()} sqft</span>
                        </div>
                      </div>
                      <div class="mt-4 pt-3 border-t border-gray-100">
                        <button class="w-full text-blue-600 text-sm font-medium hover:text-blue-800 transition-colors">
  ...
  // Create an HTML string version of county details for direct DOM insertion
  const renderCountyDetailsHTML = () => {
      ...
      <div class="p-6 county-details" data-county="${county.name}">
        ...
        <div class="mt-8">
          <div class="flex flex-wrap items-center justify-between mb-6">
            ...
            <div class="flex gap-3">
              <button
                class="active-listings-btn px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                  selectedStatus === 'active' 
                    ? 'bg-blue-600 text-white shadow-md' 
                    : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }"
              ...
              <button
                class="closed-listings-btn px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                  selectedStatus === 'closed' 
                    ? 'bg-blue-600 text-white shadow-md' 
                    : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }"
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ${properties.map(property => `
              ...
              >
                ...
                <div class="p-4">
                  <p class="text-sm text-gray-600 truncate">${property.UnparsedAddress}</p>
                  <div class="mt-3 flex items-center justify-between text-sm">
                    <div class="flex gap-4">
                      <div class="flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-blue-500 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  // Add the missing renderCountyDetails function
  ...
  // Adjust map rendering and composition
  return (
    <div className="flex flex-col w-full">
      {/* Map Section - Fixed Centering */}
      <div className={`map-container relative ${mapLoaded ? 'opacity-100' : 'opacity-0'}`}
           style={{ height: '100%', transition: 'opacity 1s ease-in-out' }}>
        <ComposableMap
          projection="geoMercator" // Changed from geoAlbersUsa for better control
          projectionConfig={{
            scale: 3450,
            center: [-77.85, 38.55] // Set center directly in projection config
          }}
          style={{
            width: '100%',
            height: '100%'
        ...
        >
          <ZoomableGroup 
            center={[0, 0]} // Reset to default since we're setting center in projectionConfig
            zoom={1.0} // Standard zoom
            translateExtent={[
              [-200, -200],
              [1000, 800]
            ]} // Set pan boundaries
      ...
      {/* County Details Section - Made more visible with clear styling */} 
            {selectedCounty && (
              <div id="map-county-details-section" 
                  className="county-details-section bg-white border-t border-gray-200 mt-4 overflow-visible"
