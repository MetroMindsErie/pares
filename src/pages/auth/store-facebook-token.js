...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Get Facebook user ID from profile if not provided
    let fbUserId = provider_user_id;
    if (!fbUserId) {
      try {
        // Use the token to get user info from Facebook
        ...
        // Continue anyway using the provider_user_id from metadata if available
        fbUserId = userData.user.user_metadata?.provider_id || 'unknown';
      }
    }

    // Store token in auth_providers table
    ...
    if (providerError) {
      ...
      // Continue to try the users table anyway
    }

    // Also store in users table for redundancy
