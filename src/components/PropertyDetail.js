...
const PropertyDetail = ({ property, onClose }) => {
  ...
  useEffect(() => {
    if (user?.roles?.includes('crypto_investor')) {
      console.log('Setting cryptoInvestorSelected flag based on user object role');
    ...
    if (user && getUserRole() === 'crypto_investor') {
      console.log('Setting cryptoInvestorSelected flag based on getUserRole()');
      localStorage.setItem('cryptoInvestorSelected', 'true');
      setIsCryptoInvestor(true);
      roleCheckCompleted.current = true;
      setIsLoading(false);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (params.get('force') === 'crypto') {
        console.log('⚠️ Forcing CRYPTO template via URL parameter');
        setForceMode('crypto');
        setIsCryptoInvestor(true);
        setIsLoading(false);
      }
  ...
  useEffect(() => {
    const detectCryptoInvestorRole = async () => {
      ...
      try {
        console.log('PropertyDetail: Starting comprehensive role check');
        
        // Direct database check (most reliable)
        console.log('Performing direct database check for crypto role');
        ...
        if (!error && data?.roles) {
          ...
          console.log('Database check result:', hasCryptoRole ? 'HAS CRYPTO ROLE' : 'NO CRYPTO ROLE');
  ...
  useEffect(() => {
    ...
    console.log('PropertyDetail rendering decision:');
    console.log('- isCryptoInvestor state:', isCryptoInvestor);
    console.log('- forceMode:', forceMode);
    console.log('- localStorage flag:', localStorage.getItem('cryptoInvestorSelected'));
    ...
    console.log('⭐ Will render:', templateType, 'template');
